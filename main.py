...
u"\u03B1"+')% of the candidate haplotypes, so that at most 1 + 100'+u"\u00D7"+'(1-'+u"\u03B1"+')% of the founder haplotypes remain after pruning (default is the original scheme using maximum RL).', width=64))]+
                ...
                ['\n'])
                optlist, args = getopt.gnu_getopt(sys.argv[1:], 'he:k:r:a:w:tv:', ["help", "error=", "kappa=", "rho=", "aggressive=", "warmup=", "top", "verror=", "mmq=", "mbq=", "maxIS=", "redose", "filter", "skip", "impute"])
                valid_options = set([("-a", "--aggressive"),("-h", "--help"), ("-w", "--warmup"), ("--error", '-e'), ("--kappa", '-k'), ("--rho", '-r'), ("-t","--top"), ("--mmq", ), ("--mbq", ), ("--maxIS", ), ("-v", "--verror"), ("--redose", ), ("--filter", ), ("--skip", ), ("--impute", )])	
                ...
                opts = [None, '1.5e-02', 0.7, 3000, 13, 20, 3e-01, 0.4, 0]	# default values for aggressive pruning rate, base calling error, pruning rate, maxIS, minimum base quality, minimum mapping quality, branching threshold, variant inconsistency rate and warm-up length.
                for _n, _pair in enumerate(valid_options):
                        ...
                        try:
                                opts[_n] = float(_lst[0]) if _n<3 or _n==6 or _n==7 else int(_lst[0])
                                ...
                                if "could not convert string to float" in e.args[0] or "invalid literal for int" in e.args[0] or "invalid literal for float" in e.args[0]:
                                        ...
                                        raise
                                raise
                alpha, error_rate, k, maxIS, mbq, mmq, rho, v_error, warmup = opts
